...
def dashboard():
    ...
    sexo_db = current_app.db.session.query(Paciente.sexo, func.count(Paciente.id_paciente))\
        .group_by(Paciente.sexo).all()
    conteo_sexo = {s: c for s, c in sexo_db}

    grados_db = current_app.db.session.query(Diagnostico.grado_retinopatia, func.count(Diagnostico.id_diagnostico))\
